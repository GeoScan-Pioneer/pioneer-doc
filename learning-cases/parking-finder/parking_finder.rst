...
* **new_command** - если True, то это значит, что коптеру необходимо лететь в новую точку;

* **leds_sent** - если True, то это значит, что команда на включение/отключение светодиодов была отправлена. Переменная нужна для единоразовой отправки таковой команды, чтобы не слать значение светодиодов постоянно;
...
Внутри бесконечного цикла на строках 25-31 происходит:

* считывание сырого изображения с камеры квадрокоптера **(25)**,
* преобразование его в понятный OpenCV формат **(26)**,
* преобразование изображения из цветового пространства BGR (Blue Green Red), используемого в OpenCV в пространство RGB (Red Green Blue), используемого нейросетью **(28)**
* а также преобразование RGB-зображения в формат PIL (библиотека для работы с изображениями) **(29)**
* и наконец получение предсказания модели **(31)**

.. literalinclude:: ./parking_finder/main.py
   :language: python
   :lines: 25-31

На строках 33-79 происходит считывание и обработка нажатий клавиш клавиатуры, а также отправка команды квадрокоптеру на движение
в новую точку:
...
На строке 81-82 располагается отрисовка текста ка показываемом на экране компьютера изображении, в котором содержится наименование
...
На строках 84-100 происходит установка значений светодиодов на коптере. Логика у условий уследующая: проверяется имя предсказанного
...
Последними строками идут отображение видео с коптера на экране ноутбука **(102)** и закрытие всех окон после выхода из цикла **(104)**