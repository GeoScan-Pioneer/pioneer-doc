...
При подаче питания Пионер определяет свое местоположение. Эта точка принимается за начало координат при выполнении команды ap.goToLocalPoint. Направления осей обозначены пиктограммой на плате модуля GPS (ось X - вперед, ось Y - влево).
