...
Что версия автопилота: 1.6.7482. Проверить можно в Pioneer Station. Для этого подключите квадрокоптер по USB, в правом нижнем углу программы в строчке "Версия АП" должно быть 1.6.7257. При необходимости обновите,
...
:download:`ESPTOOL 2.0 с прошивкой ESP32 0.2.7 <media/esp32/ESPTOOL-2.0-17022021.zip>` - обновлённая версия для работы с pioneer_sdk и Python с параметрами 0012

.. attention:: после перепрошивки ESP32 будет необходимо вручную вернуть параметр Umax в значение 3. В скором времени выпустим патч, который будет делать это автоматически.


:download:`ESPTOOL с прошивкой ESP32 0.2.7 <media/esp32/ESPTOOL0.2.7.zip>` - встроены параметры 0007 (Устарело)
